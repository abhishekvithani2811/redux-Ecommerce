...
import { DecreaseQuantity, IncreaseQuantity, getCartData, total } from '../redux/actions/productsAction2';
...
const AddToCart = () => {
  ...
  const FetchProduct = async () => {
    ...
    // const total1 = response.price * response.quantity
                  ...
                  <div class="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                    <div class="flex w-2/5">
                      ...
                      <div class="flex flex-col justify-between ml-4 flex-grow">
                        ...
                        {/* <a href="#" class="font-semibold hover:text-red-500 text-gray-500 text-xs underline	" onClick={() => RemoveProd(item.id)}>Remove</a> */}
                      </div>
                    </div>
                    <div class="flex justify-center w-1/5">
                      <svg class="fill-current text-gray-600 w-3" viewBox="0 0 448 512" onClick={() => handleDecreaseQuantity(item.productId, item.quantity)}><path d="M416 208H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h384c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z" />

                      </svg>

                      <input class="mx-2 border text-center w-8" type="text" value={item.quantity} />

                      <svg class="fill-current text-gray-600 w-3" viewBox="0 0 448 512" onClick={() => { handleIncreaseQuantity(item.productId, item.quantity) }}>
                        <path d="M416 208H272V64c0-17.67-14.33-32-32-32h-32c-17.67 0-32 14.33-32 32v144H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h144v144c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32V304h144c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z" />
                      </svg>
                    </div>
                    <span class="text-center w-1/5 font-semibold text-sm">${item.price}</span>
                    <span class="text-center w-1/5 font-semibold text-sm">${TotalPrice(item.price, item.quantity)}</span>
                  </div>
	...
	">Apply</button>
              <div class="border-t mt-8">
                <button type='submit' class="flex font-semibold justify-between py-6 text-sm uppercase">
                  ...
                  <span>{calculateTotalCart(product) + 10}</span>
                </button>
